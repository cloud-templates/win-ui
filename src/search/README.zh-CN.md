...
```html
<win-search v-model="value" placeholder="请输入搜索关键词" />
```
...
> Tips: 在 win-search 外层增加 form 标签，且 action 不为空，即可在 iOS 输入法中显示搜索按钮
...
```html
<win-search v-model="value" disabled placeholder="请输入搜索关键词" />
```
