...
```js
import Vue from 'vue';
import { Image as VanImage } from 'win-ui';
...
```html
<win-image width="100" height="100" src="https://img.yzcdn.cn/vant/cat.jpeg" />
```
...
```html
<win-image src="https://img.yzcdn.cn/vant/cat.jpeg">
  <template v-slot:loading>
    <win-loading type="spinner" size="20" />
  </template>
</win-image>
```
...
```html
<!-- 错误写法 -->
<win-image src="./image.png" />

<!-- 正确写法 -->
<win-image :src="require('./image.png')" />
```
...
这是因为 \<image> 标签是原生的 SVG 标签，Vue 不允许将原生标签名注册为组件名，使用 \<win-image> 即可规避这个问题。
