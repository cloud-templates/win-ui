...
test('close event', () => {
  ...
  const close = wrapper.find('.win-notice-bar__right-icon');
...
test('replay event', async () => {
  ...
  wrapper.find('.win-notice-bar__content').trigger('transitionend');
  await later(50);
...
test('should scroll when content width > wrap width ', async () => {
  ...
  const wrap = wrapper.find('.win-notice-bar__wrap');
  const content = wrapper.find('.win-notice-bar__content');
