...
test('popup lock scroll', () => {
  ...
  expect(document.body.classList.contains('win-overflow-hidden')).toBeTruthy();
  ...
  wrapper2.vm.$destroy();
  expect(document.body.classList.contains('win-overflow-hidden')).toBeFalsy();
});
...
test('get container with parent', () => {
  ...
  const popup = wrapper.find('.win-popup').element;
...
test('render overlay', async () => {
  ...
  expect(div.querySelector('.win-overlay')).toBeTruthy();
});
...
test('watch overlay prop', async () => {
  ...
  await later();
  expect(div.querySelector('.win-overlay')).toBeFalsy();
...
test('close on click overlay', async () => {
  ...
  const modal = div.querySelector('.win-overlay');
...
test('closeable prop', () => {
  ...
  wrapper.find('.win-popup__close-icon').trigger('click');
