...
test('click number key', () => {
  ...
  clickKey(wrapper.findAll('.win-key').at(0));
...
test('click delete key', () => {
  ...
  clickKey(wrapper.findAll('.win-key').at(11));
  expect(wrapper.emitted('delete')).toBeTruthy();
});

test('click collapse key', () => {
  const wrapper = mount(NumberKeyboard);
  clickKey(wrapper.findAll('.win-key').at(9));
...
test('click close button', () => {
  ...
  clickKey(wrapper.findAll('.win-key').at(12));
...
test('focus on key', () => {
  ...
  const key = wrapper.find('.win-key');
  trigger(key, 'touchstart');
  expect(wrapper).toMatchSnapshot();
  trigger(key, 'touchend');
  expect(wrapper).toMatchSnapshot();
...
test('bind value', () => {
  ...
  const keys = wrapper.findAll('.win-key');
  clickKey(keys.at(0));
  clickKey(keys.at(1));
...
test('show-delete-key prop', () => {
  ...
  expect(wrapper.contains('.win-key--delete')).toBeTruthy();

  wrapper.setData({ showDeleteKey: false });
  expect(wrapper.contains('.win-key--delete')).toBeFalsy();
...
test('close-button-loading prop', () => {
  ...
  expect(wrapper.contains('.win-key__loading-icon')).toBeTruthy();
});
