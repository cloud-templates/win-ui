...
<win-list
  ...
  @load="onLoad"
>
  <win-cell v-for="item in list" :key="item" :title="item" />
</win-list>
```
...
```html
<win-pull-refresh v-model="refreshing" @refresh="onRefresh">
  <win-list
    v-model="loading"
    :finished="finished"
    finished-text="没有更多了"
...
若 List 的内容使用了 float 布局，可以在容器上添加`win-clearfix`类名来清除浮动，使得 List 能正确判断元素位置
