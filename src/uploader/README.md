...
```html
<win-uploader v-model="fileList" multiple />
```
...
```html
<win-uploader disabled />
```

### Upload Status

```html
<win-uploader v-model="fileList" :after-read="afterRead" />
```
...
```html
<win-uploader v-model="fileList" multiple :max-count="2" />
```
...
```html
<win-uploader>
  <win-button icon="photo" type="primary">Upload Image</win-button>
</win-uploader>
```
...
```html
<win-uploader :before-read="beforeRead" />
```
