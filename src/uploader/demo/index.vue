<template>
  <demo-section>
    <demo-block :title="t('basicUsage')">
      <win-uploader :after-read="afterRead" />
    </demo-block>

    <demo-block :title="t('preview')">
      <win-uploader v-model="fileList" multiple accept="*" />
    </demo-block>

    <demo-block :title="t('disabled')">
      <win-uploader :after-read="afterRead" disabled />
    </demo-block>

    <demo-block v-if="!isWeapp" :title="t('status')">
      <win-uploader v-model="statusFileList" :after-read="afterReadFailed" />
    </demo-block>

    <demo-block :title="t('maxCount')">
      <win-uploader v-model="fileList2" multiple :max-count="2" />
    </demo-block>
    ...
    <demo-block :title="t('uploadStyle')">
      <win-uploader>
        <win-button type="primary" icon="photo">
          {{ t('upload') }}
        </win-button>
      </win-uploader>
    </demo-block>
    ...
    <demo-block :title="t('beforeRead')">
      <win-uploader v-model="fileList3" :before-read="beforeRead" />
    </demo-block>
  </demo-section>
</template>
