...
```html
<win-count-down :time="time" format="DD 天 HH 时 mm 分 ss 秒" />
```
...
/>
<win-grid clickable>
  <win-grid-item text="开始" icon="play-circle-o" @click="start" />
  <win-grid-item text="暂停" icon="pause-circle-o" @click="pause" />
  <win-grid-item text="重置" icon="replay" @click="reset" />
</win-grid>
```
