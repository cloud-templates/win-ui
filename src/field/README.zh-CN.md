...
<win-cell-group>
  <win-field v-model="value" label="文本" placeholder="请输入用户名" />
</win-cell-group>
```
...
```html
<!-- 输入任意文本 -->
<win-field v-model="text" label="文本" />
<!-- 输入手机号，调起手机号键盘 -->
<win-field v-model="tel" type="tel" label="手机号" />
<!-- 允许输入正整数，调起纯数字键盘 -->
<win-field v-model="digit" type="digit" label="整数" />
<!-- 允许输入数字，调起带符号的纯数字键盘 -->
<win-field v-model="number" type="number" label="数字" />
<!-- 输入密码 -->
<win-field v-model="password" type="password" label="密码" />
```
...
```html
<win-cell-group>
  <win-field label="文本" value="输入框只读" readonly />
  <win-field label="文本" value="输入框已禁用" disabled />
</win-cell-group>
```
...
>
  <template #button>
    <win-button size="small" type="primary">发送验证码</win-button>
  </template>
</win-field>
```
