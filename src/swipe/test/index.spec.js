...
const Component = {
  template: `
    <win-swipe ref="swipe" v-bind="$props" v-on="$listeners">
      <win-swipe-item :style="style">1</win-swipe-item>
      <win-swipe-item :style="style">2</win-swipe-item>
      <win-swipe-item :style="style">3</win-swipe-item>
    </win-swipe>
  `,
  props: {
    vertical: Boolean,
...
test('vertical swipe', () => {
  ...
  const { swipe } = wrapper.vm.$refs;
  const track = wrapper.find('.win-swipe__track');
...
test('lazy-render prop', async () => {
  const wrapper = mount({
    template: `
      <win-swipe :initial-swipe="active" lazy-render>
        <win-swipe-item><span>1</span></win-swipe-item>
        <win-swipe-item><span>2</span></win-swipe-item>
        <win-swipe-item><span>3</span></win-swipe-item>
        <win-swipe-item><span>4</span></win-swipe-item>
      </win-swipe>
    `,
    data() {
      return {
  ...
  await later();
  const items = wrapper.findAll('.win-swipe-item');
