...
<win-form @submit="onSubmit">
  ...
  <div style="margin: 16px;">
    <win-button round block type="info" native-type="submit">
      Submit
    </win-button>
  </div>
</win-form>
...
```html
<win-field name="switch" label="Switch">
  <template #input>
    <win-switch v-model="switchChecked" size="20" />
  </template>
</win-field>
```
...
```html
<win-field name="checkbox" label="Checkbox">
  <template #input>
    <win-checkbox v-model="checkbox" shape="square" />
  </template>
</win-field>
<win-field name="checkboxGroup" label="CheckboxGroup">
  <template #input>
    <win-checkbox-group v-model="checkboxGroup" direction="horizontal">
      <win-checkbox name="1" shape="square">Checkbox 1</win-checkbox>
      <win-checkbox name="2" shape="square">Checkbox 2</win-checkbox>
    </win-checkbox-group>
  </template>
</win-field>
```
...
```html
<win-field name="radio" label="Radio">
  <template #input>
...
```html
<win-field name="stepper" label="Stepper">
  <template #input>
    <win-stepper v-model="stepper" />
  </template>
</win-field>
```
...
```html
<win-field name="rate" label="Rate">
  <template #input>
    <win-rate v-model="rate" />
  </template>
</win-field>
```
...
```html
<win-field name="slider" label="Slider">
  <template #input>
    <win-slider v-model="slider" />
  </template>
</win-field>
```
...
```html
<win-field name="uploader" label="Uploader">
  <template #input>
    <win-uploader v-model="uploader" />
  </template>
</win-field>
```
...
/>
<win-popup v-model="showPicker" position="bottom">
  <win-picker
    show-toolbar
    :columns="columns"
    @confirm="onConfirm"
...
<win-field
  ...
  @click="showArea = true"
/>
<win-popup v-model="showArea" position="bottom">
  <win-area
    :area-list="areaList"
    @confirm="onConfirm"
...
/>
<win-calendar v-model="showCalendar" @confirm="onConfirm" />
```
