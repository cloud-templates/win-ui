...
test('max-range prop when type is range and showConfirm is false', async () => {
  ...
  const days = wrapper.findAll('.win-calendar__day');
...
test('show-title prop', () => {
  ...
  expect(wrapper.contains('.win-calendar__header-title')).toBeTruthy();
  wrapper.setProps({ showTitle: false });
  expect(wrapper.contains('.win-calendar__header-title')).toBeFalsy();
});
...
test('show-subtitle prop', () => {
  ...
  expect(wrapper.contains('.win-calendar__header-subtitle')).toBeTruthy();
  wrapper.setProps({ showSubtitle: false });
  expect(wrapper.contains('.win-calendar__header-subtitle')).toBeFalsy();
});
...
test('hide close icon when there is no title', () => {
  ...
  expect(wrapper.contains('.win-popup__close-icon')).toBeTruthy();

  wrapper.setProps({
    showTitle: false,
    showSubtitle: false,
  });
  expect(wrapper.contains('.win-popup__close-icon')).toBeFalsy();
});
...
test('min-date after current time', () => {
  ...
  wrapper.find('.win-calendar__confirm').trigger('click');
